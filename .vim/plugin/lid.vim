...
function! s:RunLid(cmd_name, ...)
    let usage = 'Usage: Lid [[-g] [-G word] [-x] [-X word] [-?] [-h]] [identifier]'
    ...
    let argcnt = 1
    while argcnt <= a:0
        let one_arg = substitute(a:{argcnt}, "^\s*", "", "")
        ...
        elseif one_arg == '-x'
            ...
            let exc_opt = 1
        elseif one_arg == '-G'
            " Include lines matching specified pattern
            let argcnt = argcnt + 1
            let match_pat = a:{argcnt}
        elseif one_arg == '-X'
            " Exclude lines matching specified pattern
            let argcnt = argcnt + 1
            let skip_pat = a:{argcnt}
        else
            " Lid option
    ...
    if g:LID_OpenQuickfixWindow == 1
        ...
        au BufWipeout <buffer> sign unplace *
        "if v:version >= 703
